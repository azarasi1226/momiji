...
* external_identities の主キーは `issuer` + `subject` の複合キーである。subject は同一 IdP 内でユニークだが、IdP が異なれば同じ subject が存在しうる。issuer(IdP を識別する値) を組み合わせることで、IdP を移行しても既存のレコードと衝突しない設計になっている。
