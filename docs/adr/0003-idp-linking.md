...
- [IdentityProviderResolver](../../backend/src/main/kotlin/jp/momiji/domain/idp/IdentityProviderResolver.kt) で whitelist 違反は fail-closed で `BusinessException` を投げる
- これにより:
...
### access token は auth proof 専用、 IDP 属性は admin API で取得 ( Keycloak / Cognito 共通 )

backend が IDP の判定に使う属性 ( `identity_provider` 相当 ) は **access token の claim に乗せず、 backend が IDP に直接 admin 権限で問い合わせる**。

| IDP | 問い合わせ API | 取得するもの |
|---|---|---|
| Keycloak | `GET /admin/realms/{realm}/users/{id}/federated-identity` | `[{identityProvider: "google", ...}]` |
| Cognito | `AdminGetUser` の `identities` user attribute | `[{providerType: "Google", ...}]` |

両 IDP 経路で `IdpUserClient.resolveIdentityProvider` 実装が **同一構造 ( sub 抽出 → admin API で federated identity 取得 → firstOrNull → 名前マッピング解決 )** になる。

この設計の利点:

- access token は **`sub` のみの最低限 claim** で運ばれ、 token 漏洩時の **情報露出を最小化**
- IDP ごとの **custom claim の protocol mapper 設定が不要** ( realm.json / Cognito user pool 設定がシンプルに )
- 「identity_provider 判定の責任は backend 側」 が一箇所に集約

注意: access token の `sub` claim は OIDC 仕様で必須なのは ID Token 側だけで、 OAuth 2.0 access token には default で含まれない。 Keycloak では `oidc-sub-mapper` を明示的に設定する必要がある ( momiji-realm.json の両 client に `subject` mapper として設定済 ) 。 backend が user 識別子として `JwtAuthenticationToken.token.subject` を読むため、 この mapper は **必須**。

### IDP 配列形式 ( Cognito の `identities` / Keycloak の `federated-identity` ) との関係
