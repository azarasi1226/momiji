...
import { type Card, changeDefaultCard, deleteCard } from "./actions";
...
const BRAND_STYLES: Record<string, { gradient: string; label: string }> = {
  ...
  mastercard: {
    gradient: "from-zinc-900 via-red-950 to-orange-900",
    label: "Mastercard",
  },
  jcb: { gradient: "from-emerald-800 via-teal-700 to-cyan-800", label: "JCB" },
  amex: {
    gradient: "from-teal-700 via-cyan-700 to-sky-800",
...
export function CardList({ cards }: { cards: Card[] }) {
  ...
  const displayCards = cards
    ...
    .map((card) =>
      defaultOverride
        ? { ...card, isDefault: card.id === defaultOverride }
        : card,
    );
