...
export async function prepareCardRegistration(): Promise<PrepareCardState> {
  const session = await requireValidSession()
  try {
    const client = createGrpcClient(PrepareCardRegistrationService, session.accessToken)
...
export async function deleteCard(paymentMethodId: string): Promise<CardActionState> {
  const session = await requireValidSession()
  try {
...
/** デフォルトカードを変更する。 */
export async function changeDefaultCard(paymentMethodId: string): Promise<CardActionState> {
  const session = await requireValidSession()
  try {
    const client = createGrpcClient(ChangeDefaultCardService, session.accessToken)
