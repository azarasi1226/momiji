...
function resolveActive() {
  ...
  if (!a.issuer) {
    throw new Error(
      `${PROVIDER} の issuer env (${String(PROVIDER).toUpperCase()}_ISSUER) が未設定です`,
    );
  }
  return a;
}
  ...
  if (!metadataCache) {
    metadataCache = fetch(`${active.issuer}/.well-known/openid-configuration`)
      .then(async (res) => {
        if (!res.ok) {
          throw new Error(
            `OIDC discovery に失敗しました (status ${res.status}, issuer ${active.issuer})`,
          );
        }
...
export async function buildLogoutUrl(params: {
  ...
  if (PROVIDER === "keycloak") {
    if (!meta.end_session_endpoint) {
      throw new Error(
        "Keycloak の end_session_endpoint が OIDC discovery に含まれていません",
      );
    }
