...
| `AUTH_URL` | ローカル ❌ / 本番 ✅ | ログアウト後の戻り先 (post_logout_redirect_uri / logout_uri) の基準 URL。ローカルはリクエストから推論されるため未設定可 (default `http://localhost:4000`)。本番では IdP の「許可されたサインアウト URL」と完全一致させて明示 |
