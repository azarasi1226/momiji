...
リポジトリルートに [Taskfile](https://taskfile.dev/) を用意してあるのでコレを利用する。
...
```bash
task setup # Dockerコンテナを立て、 DBをマイグレーションし、 gRPCの実装も作成したり。　とにかく初回セットアップをすべてこなす
...
ブレークポイントの設定ができなかったり不便なのでInteliJIDEから起動したほうが良いけど、コマンドを利用し以下のようにも起動できる。
```bash
