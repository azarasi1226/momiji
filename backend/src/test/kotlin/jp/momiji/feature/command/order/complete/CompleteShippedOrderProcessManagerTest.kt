 ...
 *
 * productIds の取得は [OrderProductIdsReader] を mock し、 ここではコマンド発行だけを検証する。
 */
class CompleteShippedOrderProcessManagerTest {
    private val commandGateway = mockk<CommandGateway>()
    private val orderProductIdsReader =
        mockk<OrderProductIdsReader> {
            every { read(any()) } returns listOf("product-1")
        }
    private val pm = CompleteShippedOrderProcessManager(commandGateway, orderProductIdsReader)
    ...
    fun `OrderShipped を受けて注文完了コマンドを撃つ`() {
        ...
        verify(exactly = 1) {
            commandGateway.send(
                CompleteOrderCommand(orderId = "order-1", productIds = listOf("product-1")),
    ...
    fun `コマンドが業務エラーでも例外を投げない（リトライループにしない）`() {
        ...
        verify(exactly = 1) {
            commandGateway.send(
                CompleteOrderCommand(orderId = "order-2", productIds = listOf("product-1")),
                CommandResult::class.java,
            )
        }
    }
