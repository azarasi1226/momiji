...
class ClearBasketGrpcServiceTest {
    private val commandGateway = mockk<CommandGateway>()
    private val userIdResolver = mockk<UserIdResolver>()
    private val service = ClearBasketGrpcService(commandGateway, userIdResolver)
    ...
    fun `正常系_JWT解決したuserIdで期待した Command が CommandGateway に渡る`() {
        ...
        Context
            ...
            .call {
                runBlocking {
                    service.clearBasket(clearBasketRequest { })
                }
            }

        verify(exactly = 1) {
            commandGateway.send(
                match<ClearBasketCommand> { it.userId == "test-user-id" },
