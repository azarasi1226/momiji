...
- **read model 起点のコマンドは CommandHandler で authoritative に再検証する**：
  - Fail/Complete の `productIds`（read model 由来）が予約全商品をカバーしてるか → [`OrderState.requireReservedProductsCovered`](OrderState.kt)（欠けると負在庫を焼くため例外→リトライ）。
...
- **起動元（driver）は use-case フォルダに同居**：gRPC は各 use-case、webhook/sweeper は入口として `fail/`・`recordpaid/`、reactor は `complete/`。横断の共有 State（`OrderState`/`ProductsState`）・リーダー（`OrderProductIdsReader`）は order 直下。
- **返金フローは未実装（フェーズ2）**：`RecordOrderPaid` の `needsRefund`（期限切れ後課金のバックストップ）は現状シグナルを返すのみ。
