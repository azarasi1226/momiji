 ...
 * - whitelist 違反は [resolve] が fail-closed で [UseCaseException] を投げる
 */
abstract class IdentityProviderResolver {
    ...
    /** provider name から [IdentityProvider] を解決。 whitelist 違反は fail-closed で [UseCaseException]。 */
    fun resolve(externalName: String): IdentityProvider =
        IdentityProvider.entries.firstOrNull { externalNameOf(it) == externalName }
            ?: throw UseCaseException(BusinessError("未対応のIDPからのログインです: $externalName"))
