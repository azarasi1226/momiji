...
設計判断は `docs/adr/` を参照（0001 バリデーション / 0002 エラー分類 / 0003 IDP連携 / 0004 sub マッピング / 0005 テスト方針 / 0006 Cognito / 0007 イベント型名 / 0008 プロジェクション日時は @Timestamp / 0009 EventProcessor をコードに型安全定義・subscribing/PooledStreaming で命名規約を分ける / 0010 ハードデリートを廃しライフサイクル（archive/discontinue + status）採用 / 0011 テストデータ投入ツール(seed)を gRPC command 経由・独立ソースセットで）。
